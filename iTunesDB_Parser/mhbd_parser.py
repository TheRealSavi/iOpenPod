...
def parse_db(data, offset, header_length, chunk_length) -> dict:
    ...
    database["unk1"] = struct.unpack("<I", data[offset+12:offset+16])[0] #always 1?
    ...
    database["ChildrenCount"] = struct.unpack("<I", data[offset+20:offset+24])[0]
    database["DatabaseID"] = struct.unpack("<Q", data[offset+24:offset+32])[0]
    database["unk2"] = struct.unpack("<H", data[offset+32:offset+34])[0] #always 2?
    #nothing in docs for data[34:38]
    database["unk3"] = struct.unpack("<Q", data[offset+38:offset+46])[0] #version 0x11+ unknown use
    #nothing in docs for data[46:48]
    database["unk4"] = struct.unpack("<H", data[offset+48:offset+50])[0] #version 0x19+
    ...
    language_bytes = struct.unpack("<2s", data[offset+70:offset+72])[0] #version 0x13+
    database["Lang"] = language_bytes.decode("utf-8")
    database["LibPersistID"] = struct.unpack("<Q", data[offset+72:offset+80]) #version 0x14+
