...
MHIP_FIELDS: list[FieldDef] = [
    ...
    # Classic 6.5G snapshot (2026-08-04): 0x0100 on six rows, all track_id=0.
    # Firmware parser 0x0807DD8C uses its high byte to select group handling.
    _u16("podcast_group_flag", 0x10, section_type=_S),
    # Values 0x8000 (five) and 0x8001 (one) occur only with podcast_group_flag
    # 0x0100. The same firmware parser does not visibly promote these bytes.
    ...
    # Classic 6.5G snapshot (2026-08-04): non-zero only on the six podcast
    # group headers. Values do not match this sample's track persistent IDs.
