...
def parse_mhni(data, offset, header_length, chunk_length) -> dict:
    ...
    imageName["correlationID"] = struct.unpack("<I", data[offset + 16 : offset + 20])[0]
    # maps to mhif correlationID. generates name of the file

    imageName["ithmbOffset"] = struct.unpack("<I", data[offset + 20 : offset + 24])[0]
    # where the image data starts in the .ithmb file

    imageName["imgSize"] = struct.unpack("<I", data[offset + 24 : offset + 28])[0]
    # in bytes
    ...
    imageName["imageHeight"] = struct.unpack("<H", data[offset + 32 : offset + 34])[0]
    imageName["imageWidth"] = struct.unpack("<H", data[offset + 34 : offset + 36])[0]

    imageName["unk1"] = struct.unpack("<I", data[offset + 36 : offset + 40])[0]
    # always 0

    imageName["imgSize2"] = struct.unpack("<I", data[offset + 40 : offset + 44])[0]
   ...
   # Define known image sizes and their corresponding dimensions.
    ...
    image_format = None
    tolerance_pixels = 3  # Allow a total pixel difference (height diff + width diff) of up to 3 pixels
    ...
    for size, props in known_sizes.items():
        ...
        diff_pixels = abs(imageName["estimatedPixmapHeight"] - candidate_height) + abs(imageName["estimatedPixmapWidth"] - candidate_width)
        print(f"Candidate {candidate_height}x{candidate_width} (size {size}) - diff: {diff_pixels} pixels")
    ...
    if best_candidate is not None and best_pixel_diff <= tolerance_pixels:
        ...
        print(f"Selected image format based on computed dimensions with pixel diff {best_pixel_diff}")
    else:
        print(f"No matching image format found for computed dimensions: {imageName["estimatedPixmapHeight"]}x{imageName["estimatedPixmapWidth"]}")
