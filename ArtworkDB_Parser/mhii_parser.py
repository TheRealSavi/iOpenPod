...
def parse_imageItem(data, offset, header_length, chunk_length) -> dict:
    ...
    image["unk1"] = struct.unpack("<I", data[offset + 28 : offset + 32])[0]  # always 0

    image["rating"] = struct.unpack("<I", data[offset + 32 : offset + 36])[
        0
    ]  # iPhoto rating x20

    image["unk2"] = struct.unpack("<I", data[offset + 36 : offset + 40])[0]  # always 0

    image["originalDate"] = struct.unpack("<I", data[offset + 40 : offset + 44])[0]
    # always 0 in ArtworkDB. creation timestamp of file in photo database.

    image["exifTakenDate"] = struct.unpack("<I", data[offset + 44 : offset + 48])[0]
    # always 0 in ArtworkDB. timestamp of taken time probably from exif in photo db.

    image["srcImgSize"] = struct.unpack("<I", data[offset + 48 : offset + 52])[0]
